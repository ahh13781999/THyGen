...
# Configuration
# ----------------------------
TOKENIZER_NAME = "jackaduma/SecRoBERTa"
MAX_LENGTH = 128
...
def main():
    ...
    os.makedirs(f"{BASE_DIR}/tokenized-secroberta", exist_ok=True)

    with open(f"{BASE_DIR}/tokenized-secroberta/label_mapping.json", "w") as f:
    ...
    try:
        ...
        torch.save(test_data, f"{BASE_DIR}/tokenized-secroberta/test.pt")
        ...
        for fold in tqdm(range(1, 6), desc="Processing folds"):
            ...
            torch.save(train_data, f"{BASE_DIR}/tokenized-secroberta/fold_{fold}_train.pt")
            ...
            torch.save(val_data, f"{BASE_DIR}/tokenized-secroberta/fold_{fold}_val.pt")
...
if __name__ == "__main__":
    main()
    print(f"\nTokenization complete. Output saved to: {BASE_DIR}/tokenized-secroberta/")
