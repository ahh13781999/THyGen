...
# Paths
TOKENIZED_DATA_DIR = "/content/drive/MyDrive/tokenized-cybert"
BASE_OUTPUT_DIR = "/content/drive/MyDrive/cybert-model_output"
CHECKPOINT_DIR = "/content/drive/MyDrive/cybert-checkpoints"
...
class CustomTrainer(Trainer):
    ...
    def predict_with_threshold(self, dataset, optimal_thresholds):
        ...
        for i in range(probs.shape[0]):
            ...
            for class_idx in range(len(sample_probs)):
                threshold = optimal_thresholds.get(class_idx, 0.5)  # Use dynamic thresholds
                if sample_probs[class_idx] >= threshold:
                    mask[class_idx] = True
            candidates = np.where(mask)[0]
            adjusted_preds[i] = candidates[np.argmax(sample_probs[candidates])] if len(candidates) > 0 else np.argmax(sample_probs)
...
def find_optimal_thresholds(labels, probs, minority_class_ids):
    thresholds = {}
    for class_idx in range(probs.shape[1]):
        if class_idx in minority_class_ids:
            # Only optimize thresholds for minority classes
            best_thresh = 0.5
            best_f1 = 0
            for thresh in np.arange(0.1, 0.9, 0.05):
                preds = (probs[:, class_idx] >= thresh).astype(int)
                f1 = f1_score((labels == class_idx).astype(int), preds, zero_division=0)
                if f1 > best_f1:
                    best_f1 = f1
                    best_thresh = thresh
            thresholds[class_idx] = best_thresh
        else:
            thresholds[class_idx] = 0.5  # Default threshold for majority classes
    return thresholds
...
def train_and_evaluate_fold(fold_num, label_mapping, minority_class_ids):
    ...
    # Find optimal thresholds for minority classes
    optimal_thresholds = find_optimal_thresholds(
        val_labels, val_probs, minority_class_ids
    )

    # Save thresholds for this fold
    with open(os.path.join(fold_dir, 'optimal_thresholds.json'), 'w') as f:
...
def evaluate_test_set(fold_num, label_mapping, minority_class_ids):
    ...
    # Load optimal thresholds for this fold
    with open(os.path.join(fold_dir, 'optimal_thresholds.json'), 'r') as f:
        optimal_thresholds = json.load(f)
    optimal_thresholds = {int(k): v for k, v in optimal_thresholds.items()}
