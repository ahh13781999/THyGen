...
# Configuration
# ----------------------------
TOKENIZER_NAME = "jackaduma/SecBERT"
MAX_LENGTH = 128
...
def main():
    ...
    os.makedirs(f"{BASE_DIR}/tokenized-secrobert", exist_ok=True)

    with open(f"{BASE_DIR}/tokenized-secrobert/label_mapping.json", "w") as f:
    ...
    try:
        ...
        torch.save(test_data, f"{BASE_DIR}/tokenized-secrobert/test.pt")
        ...
        for fold in tqdm(range(1, 6), desc="Processing folds"):
            ...
            torch.save(train_data, f"{BASE_DIR}/tokenized-secrobert/fold_{fold}_train.pt")
            ...
            torch.save(val_data, f"{BASE_DIR}/tokenized-secrobert/fold_{fold}_val.pt")
...
if __name__ == "__main__":
    main()
    print(f"\nTokenization complete. Output saved to: {BASE_DIR}/tokenized-secrobert/")
